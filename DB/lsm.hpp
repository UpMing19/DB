...
public:
    ...
    LSM<K, V>(unsigned long eltsPerRun, unsigned int numRuns, double merged_frac, double bf_fp, unsigned int pageSize, unsigned int diskRunsPerLevel) : _eltsPerRun(eltsPerRun),
                                                                                                                                                        _num_runs(numRuns), _frac_runs_merged(merged_frac), _diskRunsPerLevel(diskRunsPerLevel), _num_to_merge(ceil(_frac_runs_merged * _num_runs)), _pageSize(pageSize)
    {
        _activeRun = 0;
        _bfFalsePositiveRate = bf_fp;
        _n = 0;

        DiskLevel<K, V> *diskLevel = new DiskLevel<K, V>(pageSize, 1, _num_to_merge * _eltsPerRun, _diskRunsPerLevel,
                                                         ceil(_diskRunsPerLevel * _frac_runs_merged), _bfFalsePositiveRate);
        ...
        for (int i = 0; i < _num_runs; i++)
        {
            ...
            C_0.push_back(run);
            BloomFilter<K> *bf = new BloomFilter<K>(eltsPerRun, _bfFalsePositiveRate);
    ...
    vector<KVPair<K, V>> range(K &key, K &key2)
    {
        ...
        for (int i = _activeRun; i >= 0; i--)
        {
            vector<KVPair<K, V>> cur_elts = C_0->get_all_in_range(key1, key2);
            if (cur_elts.size() == 0)
                continue;
